class MainScene extends Phaser.Scene {
    ...
    create() {
        ...
        try {
            ...
            console.log("Couldn't load soul count;", e);
        }
        // Souls collected
        if (isNaN(this.souls)) {
            this.souls = 0;
        }
        // Try to track progress since last play
        let lastTime = localStorage.getItem('lastPlayed');
        let lastTimeNum = parseInt(lastTime);
        console.log(`Last Time Played: ${lastTimeNum}`);
        if(!isNaN(lastTimeNum)){
            let date = new Date();
            let numDate = date.getTime();
            let difference = numDate - lastTimeNum;
            let seconds = difference / 1000;
            console.log(`Difference: ${difference};${seconds}`);
            let soulsGained = Math.floor(seconds / 15);
